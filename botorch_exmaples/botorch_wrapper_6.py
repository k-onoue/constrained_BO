...
class Experiment:
    ...
    def run(self):
        for iteration in range(1, self.config["n_iterations"] + 1):
            ...
            best_value = self.train_y.max().item() if self.direction == "maximize" else self.train_y.min().item()
            self.best_values.append(best_value)
...
if __name__ == "__main__":
    ...
    experiment_config = {
        "initial_points": 5,
        "bounds": torch.tensor([[-4., -4.], [8., 8.]], device=device, dtype=dtype),
        "batch_size": 1,
        "num_restarts": 10,
        "raw_samples": 20,
        "n_iterations": 10,
    ...
    print(f'-----------------------------------------------------------')
    print(f'Best value: {experiment.best_values[-1]}')
    print(f'Best point: {train_x[train_y.argmax() if experiment.direction == "maximize" else train_y.argmin()]}')
