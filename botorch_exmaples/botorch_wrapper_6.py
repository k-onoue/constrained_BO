...
最大化と最小化の両方に対応するために，目的関数の値に対して方向を適用するように変更したい
最大化と最小化の両方に対応するために，目的関数の値に対して方向を適用するように変更したい
最大化と最小化の両方に対応するために，目的関数の値に対して方向を適用するように変更したい
最大化と最小化の両方に対応するために，目的関数の値に対して方向を適用するように変更したい
最大化と最小化の両方に対応するために，目的関数の値に対して方向を適用するように変更したい
...
#     from utils_benchmark_functions_2 import schubert_function
...
#     experiment_config = {
#         "initial_points": 5,
#         "bounds": torch.tensor([[-10, -10], [10, 10]], device=device, dtype=dtype),
#         "batch_size": 1,
#         "num_restarts": 10,
#         "raw_samples": 20,
#         "n_iterations": 40,
...
#     print(f'-----------------------------------------------------------')
#     print(f'Best value: {experiment.best_values[-1]}')
#     print(f'Best point: {train_x[train_y.argmax()]}')
...
class Experiment:
    ...
    def generate_initial_data(self, n):
        train_x = generate_integer_samples(self.bounds, n).float()
        train_y = self.apply_direction(self.objective_function(train_x).unsqueeze(-1))
        return train_x, train_y
    ...
    def apply_direction(self, y):
        return y if self.direction == "maximize" else -y
    ...
    def optimize_acqf_and_get_observation(self):
        ...
        new_y = self.apply_direction(self.objective_function(new_x).unsqueeze(-1))
        return new_x, new_y
    ...
    def run(self):
        for iteration in range(1, self.config["n_iterations"] + 1):
            ...
            best_value = (
                self.train_y.max().item()
                if self.direction == "maximize"
                else self.train_y.min().item()
            )
            self.best_values.append(best_value)

            print(
                f"Iteration {iteration}/{self.config['n_iterations']}: Best value = {best_value}"
            )
...
if __name__ == "__main__":
    ...
    experiment_config = {
        "initial_points": 5,
        "bounds": torch.tensor([[-4.0, -4.0], [8.0, 8.0]], device=device, dtype=dtype),
        "batch_size": 1,
        "num_restarts": 10,
        "raw_samples": 20,
        "n_iterations": 10,
        ...
        "direction": "minimize",
        "algo_params": {"beta": 2.0, "beta_h": 10.0, "l_h": 2.0},
    }
    ...
    print(f"Best value: {experiment.best_values[-1]}")
    print(
        f'Best point: {train_x[train_y.argmax() if experiment.direction == "maximize" else train_y.argmin()]}'
    )
