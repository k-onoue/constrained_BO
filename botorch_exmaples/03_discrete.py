...
def objective_function(X):
    ...
    term2 = -torch.exp(
        0.5 * (torch.cos(2 * torch.pi * x1) + torch.cos(2 * torch.pi * x2))
    )
    ...
    def adjust_parameters():
        ...
        def optimization_objective(params):
            ...
            ucb = UpperConfidenceBound(
                model, beta=adjusted_beta, posterior_transform=IdentityMCObjective()
            )
            ...
            rounded_candidate = [
                min(search_space[key], key=lambda x: abs(x - candidate[i]))
                for i, key in enumerate(search_space.keys())
            ]
            penalty = 0 if rounded_candidate not in observed_points else 1
            return (
                delta_beta
                + sum((torch.tensor(candidate) - torch.tensor(rounded_candidate)) ** 2)
                + penalty
            )
