...
def define_acquisition_function(model, search_space, beta, observed_points, optimize_beta_flag=False, beta_h=10, l_h=2):
...
def round_to_nearest_search_space_point(new_X, search_space):
    rounded_X = torch.tensor([[min(search_space['x1'], key=lambda x: abs(x - new_X.item()))]])
    return rounded_X
...
def main():
    ...
    for _ in range(2):
        ...
        while not success:
            ...
            acq_func, updated = define_acquisition_function(model, search_space, beta=0.1, observed_points=train_X.numpy(), optimize_beta_flag=True)
            new_X = optimize_acquisition_function(acq_func, bounds)
            rounded_X = round_to_nearest_search_space_point(new_X, search_space)
            new_Y = objective_function(rounded_X)
            train_X, train_Y, success = update_data(train_X, train_Y, rounded_X, new_Y)
