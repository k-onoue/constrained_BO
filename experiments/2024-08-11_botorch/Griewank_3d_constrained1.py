...
# log_filename_base = "Griewank_3d_constrained1"
...
#         g_X = torch.ones_like(diff) - diff / coeff
...
#         acqf_eval = torch.tensor(acqf_eval).reshape(-1, 1).to(X.dtype)
...
def fit_pytorch_model_with_constraint(
    ...
    def g(X, bounds):
        ...
        cand2 = abs(bounds[1, 0] - bounds[0, 1])
        coeff = max(cand1, cand2)
        coeff = torch.tensor(coeff, device=X.device, dtype=X.dtype)
