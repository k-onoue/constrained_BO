...
class Experiment:
    ...
    def run(self):
        for iteration in range(1, self.config["n_iterations"] + 1):
            ...
            logging.info(
                f"Iteration {iteration}/{self.config['n_iterations']}: Best value = {best_value}, Best x = {best_x}"
            )
...
if __name__ == "__main__":
    ...
    experiment_config = {
        "initial_points": 5,
        "bounds": torch.tensor(
            [[-4.0, -4.0], [8.0, 8.0]], device=device, dtype=dtype
        ),  # 2次元入力変数の範囲
        "batch_size": 1,
        "num_restarts": 10,
        "raw_samples": 20,
    ...
    for i, best_x, best_val, train_x in zip(
        range(1, len(experiment.best_x_values) + 1),
        experiment.best_x_values,
        experiment.best_values,
        experiment.train_x,
    ):
        print(
            f"Iteration {i}: Best value = {best_val}, Best x = {best_x}, train_x = {train_x}"
        )
