...
if __name__ == "__main__":
    test_filenames = [
        ...
        (
            "/Users/keisukeonoue/ws/constrained_BO/experiments/2024-07-21_Ackley_v1.py",
            "2024-07-21",
            "Ackley",
        ),
    ]
    ...
    for filename, expected_date, expected_function in test_filenames:
        ...
        assert (
            date == expected_date
        ), f"Test failed for {filename}: expected date {expected_date}, got {date}"
        assert (
            objective_function == expected_function
        ), f"Test failed for {filename}: expected function {expected_function}, got {objective_function}"
        print(filename)
