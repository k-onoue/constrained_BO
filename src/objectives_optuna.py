...
from .utils_benchmark_functions import (
    ackley_function,
    discretize_function,
    rosenbrock_function,
)
    ...
    def sample(self, trial):
        ...
        for i in range(self.map_shape[0]):
            for j in range(self.map_shape[1]):
                direction_matrix[i, j] = trial.suggest_categorical(
                    f"({i},{j})", directions
                )
...
class AckleyObjective(BaseObjective):
    ...
    def sample(self, trial):
        return [
            trial.suggest_uniform(f"x{i}", self.search_space[0], self.search_space[1])
...
class DiscreteAckleyObjective(AckleyObjective):
    def __init__(
        self,
        dim: int = 2,
        search_space: tuple[float, float] = (-1, 1),
        n_split: int = 50,
    ):
    ...
    def plot_optimization_trajectory(self, study: optuna.Study):
        ...
        x_2d = np.linspace(
            self.search_space[0], self.search_space[1], self.n_split * 10
        )
        y_2d = np.linspace(
            self.search_space[0], self.search_space[1], self.n_split * 10
        )
        ...
        fig.update_layout(
            title="2次元の離散化されたAckley関数と最適化の軌跡",
            scene=dict(xaxis_title="x", yaxis_title="y", zaxis_title="f(x, y)"),
        )
        fig.show()
...
class DiscreteRosenbrockObjective(RosenbrockObjective):
    def __init__(
        self,
        dim: int = 2,
        search_space: tuple[float, float] = (-1.0, 1.0),
        n_split: int = 50,
    ):
        super().__init__(dim, search_space)
        self.n_split = n_split
    ...
    def plot_optimization_trajectory(self, study: optuna.Study):
        ...
        fig.update_layout(
            title="2次元の離散化されたRosenbrock関数と最適化の軌跡",
            scene=dict(xaxis_title="x", yaxis_title="y", zaxis_title="f(x, y)"),
        )
        fig.show()
